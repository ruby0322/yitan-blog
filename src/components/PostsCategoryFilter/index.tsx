...
import type { CategoryFilterItem } from '@/utilities/queryPosts'
...
export const PostsCategoryFilter: React.FC<Props> = ({
  activeSlug,
  categories,
  query,
  totalPostCount,
    ...
    <nav aria-label="文章分類" className="border-b border-brand-border">
      <ul className="-mb-px flex gap-6 overflow-x-auto pb-px md:gap-8">
        <li className="shrink-0">
          <Link
            aria-label={`全部，${totalPostCount} 篇文章`}
            className={cn(
              'inline-flex items-center gap-2 whitespace-nowrap border-b-2 pb-3 text-sm transition-colors md:text-base',
              isAllActive
          ...
          >
            全部
            <CategoryCountLabel active={isAllActive} count={totalPostCount} />
          </Link>
        </li>
        {categories.map((category) => {
          ...
          return (
            <li className="shrink-0" key={category.slug}>
              <Link
                aria-label={`${category.title}，${category.postCount} 篇文章`}
                className={cn(
                  'inline-flex items-center gap-2 whitespace-nowrap border-b-2 pb-3 text-sm transition-colors md:text-base',
                  isActive
                    ? 'border-brand-sage font-medium text-brand-heading'
              ...
              >
                {category.title}
                <CategoryCountLabel active={isActive} count={category.postCount} />
              </Link>
            </li>
          )
        })}
